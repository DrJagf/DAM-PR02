...
public class FacturaLineaController {
	...
	public ArrayList<FacturaLinea> listByIdFactura(Integer id ){
		
		ArrayList<FacturaLinea> list = new ArrayList<FacturaLinea>();
		for (FacturaLinea facturaLinea : repository.findAll()) {
			if (facturaLinea.getId_factura().equals(id)) {
				list.add(facturaLinea);
			}
		}
